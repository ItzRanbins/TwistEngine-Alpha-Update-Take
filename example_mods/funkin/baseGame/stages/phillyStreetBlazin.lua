function onCreate()
    ...
    if lowQuality == false then
        ...
        makeLuaSprite('sky', 'streets/blazin/skyBlur', -600, -175)
        scaleObject('sky', 1.75, 1.75)
        setScrollFactor('sky', 0, 0)
        addLuaSprite('sky')
    end
    
    makeAnimatedLuaSprite('lightning', 'streets/blazin/lightning', 50, -300)
	addAnimationByPrefix('lightning', 'anim', 'lightning0', 24, false)
    scaleObject('lightning', 1.75, 1.75)
	setScrollFactor('lightning', 0, 0)
	addLuaSprite('lightning')
    setProperty('lightning.visible', false)

    makeLuaSprite('streets', 'streets/blazin/streetBlur', -600, -175)
    scaleObject('streets', 1.75, 1.75)
    setScrollFactor('streets', 0, 0)
	addLuaSprite('streets')

    if lowQuality == false then
        makeLuaSprite('streetsMultiply', 'streets/blazin/streetBlur', -600, -175)
        ...
        makeGraphic('lightenAdd', 2500, 2000, 'FFFFFF')
...
function onCreatePost()
    ...
    if isRunning('custom_events/Set Camera Target.lua') then
        ...
        runHaxeCode('FlxG.camera.target = null;')
    end
